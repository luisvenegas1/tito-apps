 ...
 *
 * Hacía falta porque no todo lo que Supabase tira tiene `.message`: a veces
 * viene un objeto plano, y `String(objeto)` termina mostrando "{}" o
 * "[object Object]" — que es exactamente lo inútil que vio el usuario.
 */

/** Errores de Supabase que conviene explicar en criollo. */
...
function raw(err: unknown): string {
  ...
  if (err && typeof err === "object") {
    ...
    // Último recurso: que al menos se vea algo distinto de "{}".
    try {
