...
export async function signUpWithUsername(
  ...
  if (data.session && data.user) {
    const { error: upErr } = await supabase.from("profiles").update({ username }).eq("id", data.user.id);
...
/** Cambio de username desde Ajustes (mantiene unicidad). */
...
export async function requestPasswordReset(email: string): Promise<void> {
  ...
  // No revelamos si el email existe (anti-enumeración): la UI muestra mensaje genérico.
}
